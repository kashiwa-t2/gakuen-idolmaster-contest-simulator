...
// import { SkillCard } from "../types/skill-card"; // 必要に応じてパスを修正してください
...
const main = () => {
  console.log("コンテスト開始");
  const deck = new Deck(idol.skillCards);
  ...
  while (remainingTurns > 0) {
    ...
    // 山札(idol.skillcards)から3枚カードを引く
    const drawnCards = deck.drawCards();
    console.log(`ターン${currentTurn}で引いたカード: ${drawnCards}`);
    ...
    while (idol.remainingActions > 0) {
      ...
      const selectSkillCard = drawnCards[0]; // 引いたカードのうち1枚を選択
      // このターンの属性はボーカル
      const attribute: Attribute = "vocal";
      const attributeRate = getAttributeRate(idol, attribute);

      skillCards[selectSkillCard](idol, attributeRate);
      console.log(`ターン${currentTurn}: ${selectSkillCard}を実行しました`);
